...
def classify_phases(df: pd.DataFrame) -> list[str]:
    ...
    # Label post-release return-to-home frames as "return" instead of "approach".
    if release_frames:
        last_release_end = min(n, release_frames[-1] + TRANSITION_WINDOW + 1)
        for i in range(last_release_end, n):
            phases[i] = "return"
